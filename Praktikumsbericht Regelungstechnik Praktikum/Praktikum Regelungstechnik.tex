...
In diesem Praktikumsteil geht es darum, einen Roboterarm am Computer mit Hilfe von Mathlab zu simulieren und zu analysieren. 
% Der Satz ist halt einfach viel besser, Chris , danke m8 ;)
...
Dies entspricht einem Drehmoment von 44,1402 \textit {Nm}. Dieses Drehmoment wird im Modell als konstante Eingangsgröße angegebenen. Der resultierende Winkel wird dann in Grad angegeben und sollte entsprechend der Erwartung einen Winkel von 40\,$^\circ$ entsprechen.
...
An dem Diagramm ist zu sehen, dass sich die Auslenkung durch das Drehmoment \textit{u\textsubscript{0}} einstellt. Diese Auslenkung entspricht im Mittel einem Wert von ca. 0,6971 \textit{Rad}. Womit die These aus dem Skript bestätigt ist.
...
Die Wurzelortskurve lässt zur Realisierung dabei nur einen Regler zu, der zwei Nullstellen hat. Da dieser abschließend noch realisierbar sein soll, muss der Regler weiterhin mindestens drei Polstellen besitzen. Es ergibt sich folgendes Reglerbild in der Wurzelortskurve:


\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{2019-08-15_20h07_12.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{2019-08-15_20h07_22.png}
...
Der K-Wert (Verstärkungsfaktor des Reglers) sollte dabei so gewählt werden, dass die Pole alle im linken Teil der Kurve liegen und den gegebenen K-Wert von 10 nicht überschreitet. 
...
\subsection{Aufgabe 5 Drehzahlregelung}
...
Wird die Sprungantwort mit eingeschaltetem Filter aufgenommen, so fallen signifikante Unterschiede auf. Das Ansteigen der Sprungantwort verläuft nur unwesentlich langsamer dafür ergibt sich ein im Verhältnis zur Anfangsmessung geglättetes Signal, die Restwelligkeit  beträgt nur noch $\pm$ 10. Daraus kann die Übertragungsfunktion durch ein PT1-Glied angenähert werden. Es gilt dabei jedoch zu beachten, dass die Übertragungsfunktion lediglich als Näherung angesehen werden kann, da die wirkliche Sprungantwort um ca 50 überschwingt. Die so bestimmte Näherung lautet: 
...
Auf Basis der angenäherten Übertragungsfunktion kann nun ein Simulink Modell erstellt werden (in Figure 9 dargestellt). Durch dieses kann ein Regler ausgelegt werden, um ihn im realem Versuchsaufbau einzusetzen.
...
Die Vorgabe für den Regler soll eine \textit{t\textsubscript{5\%}}-Zeit von 0,3 Sekunden sein. Mithilfe einer Faustformel kann nun der Realteil des Pols bestimmt werden. Es ergibt sich dafür ein Realteil von -10 für den Pol des geschlossenen Regelkreises des P-Reglers. Der geschlossene Regelkreis weist folgende Form auf: 	
...
Der P-Regler wird nun am Simulink Modell getestet. Der Sollwert betägt 1600\textdegree/s , als Stationärer Endwert wird 1 angestrebt. Um diesen zu erreichen wird eine Vorsteuerung mit dem Verstärkungsfaktor  \textit{K\textsubscript{vor}} = $\frac{1600}{1581,5}$ implementiert. 
Die benötigte Stellgröße liegt über 5 \textit{Volt} aber noch im realistischen Bereich, dennoch liegt diese über 0,3 Sekunden lang an, sodass hier Handlungsbedarf bestüde, falls das reale System einen Error Code ausgegeben würde.
...
Zu guter letzt soll nun das Experiment auf eine zweite variable Raumdimension \textit{x\textsubscript{w}} ausgeweitet und auf eine Wunschposition eingeregelt werden. 
%(Hier Bild der Regelstruktur einfügen?)
Eine mögliche aber nicht zu empfehlende Verbesserung wäre das aufbauen eines PID-Reglers. Der Integrale Anteil unseres Systems würde die gesamt Regelung verlangsamen. Der mögliche Gewinn dieses Reglers wäre eine Zunahme der stationären Genauigkeit. Allerdings erfüllt das gesamt System diese Anforderungen schon. Es kann also festgehalten werden, dass die Kranregelung bereits annehmbar gut im Modell funktioniert.
Mögliche Verbesserungen am Versuchsaufbau sehen wir in der maximalen Leistung der Motoren, um stärker auf Abweichungen reagieren zu können, sowie eine Verringerung der Haft- bzw. Gleitreibung des Krans, um feiner und weniger verlustbehaftet arbeiten zu können.
Des Weiteren ist eine konstante Beschränkung der Motorleistung zur Berücksichtigung des Auslenkungswinkels für alle Positionen nicht die optimale Lösung, eine positionsabhängige Regelung wäre möglich.
%äääähhhmmmm: Jedoch ist der Gesamtimpuls, den die Masse bei Regelung in zwei Raumdimensionen erhält abhängig von der Wurzel der Summe der Einzelimpulsquadrate und somit unter %Umständen größer als der bisherige Maximalimpuls in y-Richtung. Dies kann zu einer größeren Gesamtauslenkung führen 
