...
\item Bestimmen der P-Regelkoeffizienten dieser Anordnung:
Bei der Anordnung können die der beiden Regler hintereinander gilt folgende Formel und folgende Formel aus der Vorbereitung

Allgemein gilt für die Formel:
\begin{align}
G_{w} = \frac{\frac{K_p * K_s*K_D}{T}}{s^2+(\frac{K_D*K_P}{T}+\frac{1}{T})*s+\frac{K_p*K_D*K_1}{T}}
\end{align} 
\begin{align}
G_{w} = \frac{186,0496}{(s+13,46) * (s-13,46)}
\end{align} 
...
Die ursprüngliche Funktion  beschreibt den offenen Regelkreis mit nur einer geregelten Funktion. Dieses System wird danach erneut mit einem PD Regler versehen kann jetzt den erweiterten Regelkreis eingebaut werden.
